...
(() => {
  ...
  function initShinsekai() {
    ...
    const TIER_CONFIG = {
      // grainSize: the film-grain overlay's background-tile size. Smaller
      // tiles look finer but cost more to rasterize (a full-viewport 3px
      // tile is a LOT of gradient tiles on a big screen). Unlike the
      // particle canvas — which already scales its resolution — this was
      // previously fixed at 3px on every tier. Scaling it down on 'mid'
      // measurably cuts paint cost on that tier.
      high: { particleCount: 30, blurPx: 16, grain: true, grainSize: 3 },
      mid:  { particleCount: 18, blurPx: 8,  grain: true, grainSize: 5 },
      eco:  { particleCount: 0,  blurPx: 0,  grain: false, grainSize: 5 }
    };
    ...
    function initCanvas() {
      ...
      function updateParticleColors() {
        ...
        if (currentThemeKey === 'crimson') {
          ...
          // Use the same legibility-clamped color as the UI chrome, so
          // particles stay visible even if the picked hex is near-black.
          const { r, g, b } = getLegibleAccent(hex);
          for (let i = 0; i < len; i++) {
            const p = particles[i];
    ...
    const CHAKRA_CRYSTALS = [
      { key: 'thunder',   kanji: '雷光・神速', rom: 'Amber Gold',      color: '#fbbf24' },
      { key: 'wind',      kanji: '風神・翠緑', rom: 'Emerald Green',   color: '#10b981' },
      { key: 'water',     kanji: '碧羅・蒼波', rom: 'Ocean Cyan',      color: '#06b6d4' },
      { key: 'void',      kanji: '虚式・紫電', rom: 'Lavender Purple', color: '#c084fc' },
      { key: 'eva',       kanji: '真紅・終焉', rom: 'Crimson Red',     color: '#ff3366' },
      { key: 'sakura',    kanji: '幽玄・紅桜', rom: 'Rose Pink',       color: '#f43f5e' },
      { key: 'flame',     kanji: '日輪・業火', rom: 'Sunset Orange',   color: '#fb923c' },
      { key: 'glacial',   kanji: '氷華・零度', rom: 'Sky Blue',        color: '#38bdf8' },
      { key: 'venom',     kanji: '毒蝶・幻紫', rom: 'Magenta Purple',  color: '#d946ef' },
      { key: 'starlight', kanji: '星骸・白金', rom: 'Platinum White',  color: '#e2e8f0' }
    ];
    ...
    function applyTheme(themeKey, syncScene = false) {
      ...
      // Clear any chakra crystal preset left highlighted from a previous
      // custom-color selection — otherwise it stays visually "active" even
      // though a default theme is now applied.
      updateActiveCrystalState('__none__');
